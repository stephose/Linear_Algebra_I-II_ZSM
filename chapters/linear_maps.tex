...
\begin{align}
	...
	\label{eq:comp_mat_b}
	S(w_j) &= \sum_{i=1}^{p} b_{ij} u_i \qquad \forall 1 \leq j \leq m\\
	\label{eq:comp_mat_c}
...
\begin{align*}
	S \circ T(v_j) &= S\big(\sum_{k=1}^{m} a_{kj} w_k\big) = \sum_{k=1}^{m} a_{kj} S(w_k) = \sum_{k=1}^{m} a_{kj} \big(\sum_{i=1}^{p}b_{ik} u_i \big)\\
