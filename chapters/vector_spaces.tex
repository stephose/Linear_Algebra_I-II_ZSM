...
\begin{lemma}{D}{lemma_d}
	Let $v_1, \hdots, v_n \in V$ be s.t. $\operatorname{Sp}(v_1, \hdots , v_n) = V$, and let $u_1, \hdots , u_m \in V$ be linearly independent. Then $m \leq n$. Moreover, one can delete $m$ vectors from the list $v_1, \hdots , v_n$ s.t. the remaining list, say $v_1', \hdots , v_n'$ together with $u_1, \hdots , u_m$ still span $V$, i.e., $\operatorname{Sp}(u_1, \hdots , u_m, v_1', \hdots , v_n') = V$.
\end{lemma}
...
\begin{theorem}{Rank Theorem}{rank_thm}
...
\begin{proof}
	Define $n := \dim V$. Let $u_1, \hdots , u_k$ be a basis for $\ker(T)$. Note that $\ker(T)$ is finite dimensional, b.c. it is a subspace of $V$, which is finite dimensional. Extend $u_1, \hdots , u_k$ to a basis $u_1, \hdots , u_k, v_1, \hdots , v_{n-k}$ of $V$. By Lemma \ref{lem*span_basis}, we have that
	\[
