...
\begin{theorem}{\textcolor{red}{Canonical 'Almost' Isomorphism between $V$ and $V^*$}}{can_iso_dual_sp}
	Let $(V, \langle \cdot, \cdot \rangle)$ be a finite dimensional inner product space over $K = \mathbb{R}$ or $\mathbb{C}$. Let $\varphi \in V^{\ast}$. Then there exists a \textbf{unique} $u \in V$ s.t. $\varphi = \varphi_u$, i.e., $\varphi(v) = \langle v, u \rangle \quad \forall \, v \in V$.
...
\begin{proof}
	Let $n := \dim V$ and fix an orthonormal basis $e_1, \hdots , e_n$ for $V$, which exists by the Gram-Schmidt orthogonalization process \ref{theo*gram_schm_orth_pro}. Let $\varphi \in V^*$. Let $v \in V$. By Theorem \ref{theo*prop_orth_sys} we have that
	\[
		v= \sum_{i=1}^{n} \langle v, e_i \rangle e_i.
	\]
	Therefore,
	\[
		\varphi(v) = \varphi \Big(\sum_{i=1}^{n} \langle v, e_i \rangle e_i\Big) = \sum_{i=1}^{n} \langle v, e_i \rangle \, \varphi(e_i) = \langle v, \sum_{i=1}^{n} \overline{\varphi(e_i)}\, e_i \rangle.
	\]
	Define 
	\[
	...
	\[
		\langle v, u_1 \rangle = \langle v, u_2 \rangle \qquad \forall v \in V.
	\]
	By a previous result, this implies that $u_1 = u_2$. This shows that $\Phi$ is surjective and injective.
	
	For $K = \mathbb{R}$, we have that the scalar product $\langle \cdot , \cdot \rangle$ is bilinear. So it follows that $\Phi$ is also linear, hence $\Phi$ is an isomorphism.
	
	For $K = \mathbb{C}$, we have that the hermitian product $\langle \cdot , \cdot \rangle$ is $\mathbb{C}$-anti linear in the second variable. So, it follows that $\Phi$ is also $\mathbb{C}$-anti linear.
\end{proof}
...
\begin{proposition}{Matrix Representation of Adjoint Maps}{mat_adj_map}
	Let $V, W$ be finite dimensional inner product spaces and $T:V \to W$ a linear map. Let $\mathcal{B} = (e_1, \hdots , e_n)$ be an orthonormal basis for $V$ and $\mathcal{C} = (f_1, \hdots, f_m)$ an orthonormal basis for $W$. Then
	\[
...
\begin{corollary}{}{cor_mat_adj_map}
	Let $A \in M_{m \times n}(K)$, where $K = \mathbb{R}$ or $\mathbb{C}$ and let $T_A:K^n \to K^m$ be the linear map $T_A(v) = Av$. Endow $K^n$ and $K^m$ with their standard inner product. Then
	\[
		(T_A)^* = T_{A^*}.
	\]
\end{corollary}
