...
\begin{proof}
	...
	\[
		\ell \circ T:V \to K
	\]
	is a linear map, b.c. both $\ell$ and $T$ are linear. So, $T^*(\ell) := \ell \circ T \in V^*$. This shows that $T^*:W^* \to V^*$ indeed has as it's target $V^*$ as claimed, see also the Image \ref{fig:dual_map}.
	\begin{figure}[h]
		\label{fig:dual_map}
		\centering
	...
	\begin{align*}
		T^*(\ell_1 + \ell_2)(v) &= \big((\ell_1 + \ell_2) \circ T\big)(v) = (\ell_1 \circ T + \ell_2 \circ T)(v) = (\ell_1 \circ T)(v) + (\ell \circ T)(v)\\
...
\begin{proof}
	...
	\begin{align*}
				S^*w_j^* &= w_j^* \circ S = \sum_{i=1}^{n} (w_j^* \circ S)(v_i) \cdot v_i^* = \sum_{i=1}^{n} w_j^*\big(S(v_i)\big)\cdot v_i^*\\
				 &= \sum_{i=1}^{n}w_j^*\bigg(\sum_{k=1}^{m}a_{ki} w_k\bigg)\cdot v_i^* = \sum_{i=1}^{n} a_{ji} v_i^*.
	\end{align*}
...
\begin{proof}
	...
	\textbf{2) $\overline{T}$ is injective:} Let $x \in \ker(T)$. Write $x$ as $x = [v]$ for some $v \in V$. Then
	\[
