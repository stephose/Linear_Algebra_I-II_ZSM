...
\begin{proof}
	...
	\end{equation}
	We need to show that $a_1 = \hdots = a_n = a_{n+1} = 0$. Apply $T$ to Equation \eqref{eq:distinct_eigenval_1}. Then we have that
	\[
...
\begin{proof}
	...
	\end{equation}
	and $[id_V]^{\mathcal{B}}_{\mathcal{B}'} = \big([id_V]^{\mathcal{B}'}_{\mathcal{B}}\big)^{-1}$. So, by Equation \eqref{eq:trace_1} we have that
	\[
		[T]^{\mathcal{B}'}_{\mathcal{B}'} = [T]^{\mathcal{B}}_{\mathcal{B}}. \qedhere
	\]
\end{proof}
...
\begin{lemma}{Coefficients of the Characteristic Polynomial}{coeff_char_poly}
	...
	In other words,
	\[
		p_A(x) = (-1)^n x^n + (-1)^{n-1}x^{n-1} + c_{n-2} x^{n-2} + \hdots + c_1 x + \det(A).
	\]
\end{lemma}
...
\begin{proposition}{\textcolor{red}{Inequlity Regarding Geometric \& Algebraic Multiplicity}}{ineq_geom_alg_multi}
	\[
